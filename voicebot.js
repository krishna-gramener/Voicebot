  ...
  const response = await fetch("https://llmfoundry.straive.com/openai/v1/realtime/sessions", {
    ...
    body: JSON.stringify({
      ...
      tools: [
        {
          name: "alertResponse",
          description: "Alerts the user with the response when user asks to",
          type: "function",
          parameters: {
            type: "object",
            properties: {
              message: {
                type: "string",
                description: "The message to be alerted",
              },
            },
            required: ["message"],
          },
        },
        {
          ...
          description: "console logs the user with the response when user asks to",
          type: "function",
          parameters: {
            type: "object",
            properties: {
...
document.querySelector("#start").addEventListener("click", async (e) => {
  ...
  dc.addEventListener("message", (e) => {
    ...
    if (d.type === "response.function_call_arguments.done") {
      const funcName = d.name;
      const args = JSON.parse(d.arguments);
      if (funcName === "alertResponse") {
        alert(args.message);
      }

      else if (funcName === "consoleResponse") {
        console.log(args.message);
      }
      else if (funcName === "searchYoutube") {
        const url = `https://www.youtube.com/results?search_query=${args.query}`;
...
${Object.values(responses).map((r) =>
          (r?.output ?? []).map((o) => (o.content ?? []).map((c) => html`<p>${c.text ?? c.transcript}</p>`))
        )}</pre
      >
    `,
    $transcript
